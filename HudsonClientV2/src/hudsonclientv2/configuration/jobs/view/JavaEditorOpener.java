...
//		MessageDialog.openInformation(shell, "Klasse nicht gefunden.", "Konnte Klasse " + clazzName + " nicht finden.");
	    } else if (compilationUnits.size() == 1) {
		return compilationUnits.get(0);
	    } else {
//		MessageDialog.openInformation(shell, "Precision necessaire.", "Plusieurs classes correspondent à " + clazzName + ".");
	    }
	} catch (Exception exception) {
	    HudsonPluginLogger.logException(exception);
	}
